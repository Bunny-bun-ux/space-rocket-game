(() => {
  ...
  const keys = new Set();
  let spaceHeld = false;
  function isQuitConfirmOpen() {
    return !quitConfirmLayer.hidden;
  }

  function showQuitConfirm() {
    quitConfirmLayer.hidden = false;
  }

  function hideQuitConfirm() {
    quitConfirmLayer.hidden = true;
  }
  ...
  function setMenuGroup(mode) {
    howtoBox.hidden = mode !== "start";
    menuStart.hidden = mode !== "start";
    menuPause.hidden = mode !== "paused";
    menuGameover.hidden = mode !== "gameover";
    if (overlayCard) {
      overlayCard.dataset.scene =
        mode === "start" ? "start" : mode === "paused" ? "pause" : mode === "gameover" ? "over" : "playing";
    }
  }

  function syncOverlayForMode() {
    if (gameMode === "start") {
      overlayTitleEl.textContent = "Space Shooter";
      overlaySubtitleEl.textContent = "Welcome! Ready to launch?";
      setMenuGroup("start");
    } else if (gameMode === "gameover") {
      overlayTitleEl.textContent = "Game Over";
      overlaySubtitleEl.textContent = `Your score: ${state.score}`;
      setMenuGroup("gameover");
    } else if (gameMode === "paused") {
      overlayTitleEl.textContent = "Game Paused";
      overlaySubtitleEl.textContent = "Take a break. Your run is saved.";
      setMenuGroup("paused");
    } else {
      // playing — overlay will hide; prep default layout for return from game
      setMenuGroup("start");
    }
  }
  ...
  playBtn.addEventListener("click", () => {
    if (gameMode !== "start" || isQuitConfirmOpen()) return;
    startFreshRun();
  });

  playAgainBtn.addEventListener("click", () => {
    if (gameMode !== "gameover" || isQuitConfirmOpen()) return;
    startFreshRun();
  });

  resumeBtn.addEventListener("click", () => {
    if (gameMode !== "paused" || isQuitConfirmOpen()) return;
    gameMode = "playing";
    syncOverlayForMode();
    overlayEl.classList.add("overlay--hidden");
  });

  function openQuitFromCurrentMenu() {
    if (gameMode !== "paused" && gameMode !== "gameover") return;
    if (isQuitConfirmOpen()) return;
    showQuitConfirm();
  }

  quitFromPauseBtn.addEventListener("click", openQuitFromCurrentMenu);
  quitFromOverBtn.addEventListener("click", openQuitFromCurrentMenu);
